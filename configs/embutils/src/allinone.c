...
/* Ensure portability for non-glibc systems */
#if defined(__linux__)
#include <linux/vt.h>
#include <sys/syscall.h>
#endif

/* Prototypes for functions missing in older/non-standard headers */
int setdomainname(const char *name, size_t len);
int sethostname(const char *name, size_t len);

/* Required global constants referenced throughout main() */
static const char clearscreen[] = "\e[H\e[J";
...
static const char nametoolong[] = "name too long\n";
static const char mustbesuper[] = "must be superuser\n";
static const char notpermit[]   = "Operation not permitted\n";
static const char notatty[]     = "not a tty\n";
static const char y[]           = "y";
static const char quiet[]       = "-quiet";
static const char silent[]      = "-silent";
static const char illegal[]     = "command not found\n";

/* Prototypes for helper assembly/C implementations */
extern int __write1(const char *s);
extern int __write2(const char *s);
#ifdef __linux__
extern long __syscall_syslog(int type, char *buf, int len);
#endif

/* Modernized helper replacing regparm attribute */
static void disp(const char *s) {
    static int notfirst = 0;
    if (notfirst) __write1(spc);
    notfirst = __write1(s);
}

/* Updated interpret_esc to use standard types */
...
unsigned char s[4] = {0, 0, 0, 0};
    for (int i = 0; i < 4 && p[i] != '\0'; i++) {
        s[i] = (unsigned char)p[i];
    }
    
    switch (_(s[0], s[1], s[2], s[3])) {
