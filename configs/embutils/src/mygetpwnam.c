...
#else
#include <pwd.h>      /* Ensure struct passwd is defined */
#endif
...
static struct passwd *__getpw(int idx, const char *value, unsigned long value2) {
    ...
    for (i = 0; i < count; i++) {
        ...
        if (seven == 7) {
            if (((idx == 2 || idx == 3) && (unsigned long)atoi(tmp2[idx]) == value2) ||
                (idx != 2 && idx != 3 && strcmp(tmp2[idx], value) == 0)) {
                ...
                diff = (long)(tmp2[0] - p.pw_name);
                memcpy(p.pw_name, tmp[i], tmplen);
                p.pw_name[tmplen] = '\0'; /* Ensure null termination */

#ifndef DONT_NEED_PASSWD
                p.pw_passwd = tmp2[1] - diff;
#endif
#ifndef DONT_NEED_UID
                p.pw_uid = (uid_t)atoi(tmp2[2]);
#endif
#ifndef DONT_NEED_GID
                p.pw_gid = (gid_t)atoi(tmp2[3]);
#endif
