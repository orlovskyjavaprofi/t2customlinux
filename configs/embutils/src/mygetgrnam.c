...
#include <string.h>   /* Required for strlen, strcmp, memcpy */
#include <stdlib.h>   /* Required for malloc, free, atoi */
#include <stdio.h>    /* Often needed for underlying system definitions */
...
#else
#include <grp.h>      /* Ensure struct group is defined even in EMBEDDED mode */
#endif
...
static struct group *__getgr(int idx, const char *value, unsigned long value2) {
    ...
    for (i = 0; i < count; i++) {
        char **tmp2;
        int four;
        size_t tmplen = strlen(tmp[i]);
        tmp2 = split(tmp[i], ':', &four, 0, 0);
        
        if (four == 4) {
            if ((idx == 2 && (unsigned long)atoi(tmp2[idx]) == value2) ||
                (idx != 2 && strcmp(tmp2[idx], value) == 0)) {
        ...
        free(tmp2); /* Fixed memory leak: tmp2 must be freed in each iteration */
    }
    free(tmp);
    free(buf);
    return NULL;
