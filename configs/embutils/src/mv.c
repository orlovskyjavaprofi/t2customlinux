...
/* Forward Declarations for GCC 16 compliance */
...
unsigned int str_rchr(const char *in, char needle) {
  ...
  return (unsigned int)((found ? found : t) - in);
}

void verbose(const char* src, const char* dest) {
  if (v) {
    char *tmp = alloca(strlen(src) + strlen(dest) + 50);
    int len = str_copy(tmp, "\"");
...
int perms(const char *file, struct stat *s) {
    ...
    if (chown(file, s->st_uid, s->st_gid))
        if (euid == 0) { res = 1; oops2("chown", file); }
    return res;
}
...
int copymove(const char *src, const char *dest) {
  ...
  if (stat(src, &ss)) {
    char buf[1024];
    int rlink = readlink(src, buf, 1024);
    if (rlink >= 0) {
      buf[rlink] = 0;
      if (symlink(buf, dest)) { oops2("symlink", dest); return -1; }
  ...
  if (S_ISDIR(ss.st_mode)) {
    ...
    DIR *D;
    if (!r) {
      char *d_msg = alloca(strlen(dest) + 100);
      int len = str_copy(d_msg, name);
      len += str_copy(d_msg + len, dest);
      str_copy(d_msg + len, " is a directory.\n");
      __write2(d_msg);
      return 0;
    }
    ...
    while ((d = readdir(D))) {
      ...
      int len;
      len = str_copy(a, src); a[len++] = '/'; str_copy(a + len, d->d_name);
      len = str_copy(b, dest); b[len++] = '/'; str_copy(b + len, d->d_name);
      verbose(a, b);
      copymove(a, b);
    }
    closedir(D);
  ...
  if (ss.st_size > (1UL << 31) || !S_ISREG(ss.st_mode)) {
normalcopy:
    {
      char buf[16 * 1024];
      ssize_t len;
...
#if defined(__linux__) && defined(SYS_sendfile)
    off_t ofs = 0;
    if (sendfile(dfd, sfd, &ofs, (size_t)ss.st_size) != ss.st_size) goto normalcopy;
#else
    goto normalcopy;
#endif
  }
...
int domove(char *src, const char *dest, int mustbedir) {
    ...
    if (stat(dest, &ss)) {
        ...
        if (I && !f) {
            ...
            write(1, " exists. Overwrite? ", 20);
            if (read(0, buf, 9) >= 1)
                if (buf[0] == 'y' || buf[0] == 'Y') ok = 1;
            if (!ok) return 0;
        }
        if (f || I || mode == CP) {
            if (!stat(src, &ss2)) {
                if (ss2.st_dev == ss.st_dev && ss2.st_ino == ss.st_ino && ss2.st_size == ss.st_size) return 0;
                else unlink(dest);
            } else if (mode == CP) unlink(dest);
        }
    }
    verbose(src, dest);
    if (mode == CP) return copymove(src, dest);
    int ret = rename(src, dest);
    if (ret < 0 && errno == EXDEV) return copymove(src, dest);
    return ret;
}

void usage(void) {
  if (mode == MV) warn("usage: mv [-fvi] source... destination");
  else warn("usage: cp [-prfvi] source... destination");
  exit(0);
}

int main(int argc, char *argv[]) {
  int k, first = 0, num = 0, last = 0;
  char *me = strrchr(argv[0], '/');
  euid = geteuid();
  mask = 0777 - umask(0);
  if (me) ++me; else me = argv[0];
  
  if (!strcmp(me, "mv")) mode = MV;
  else if (!strcmp(me, "cp")) { mode = CP; name = "cp: "; }
  else error("argv[0] has to be cp or mv.");

  if (argc < 2) usage();
  for (k = 1; k < argc; ++k) {
    if (argv[k][0] == '-') {
      for (size_t j = 1; j < strlen(argv[k]); j++) {
        switch (argv[k][j]) {
        case 'f': f = 1; break;
        case 'v': v = 1; break;
        case 'i': I = 1; break;
  ...
  switch (num) {
  case 1:
    if (domove(argv[first], ".", 0)) oops(errno != EEXIST ? argv[first] : ".");
    break;
  case 2:
    if (domove(argv[first], argv[last], 0)) oops(errno != EEXIST ? argv[first] : argv[last]);
    break;
  default:
    while (num > 1) {
      if (argv[first][0] != '-') {
        if (domove(argv[first], argv[last], 1)) oops(errno != EEXIST ? argv[first] : argv[last]);
        --num;
      }
      ++first;
    }
